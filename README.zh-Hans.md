...
| 模块 | 说明 |
|------|------|
| **视口** | 旋转、缩放、适应窗口、重置相机；双击适应；清屏预览（影院模式）与自动旋转 |
| **模型库** | 打开单个文件或扫描文件夹；搜索；格式标识；在访达中显示 |
...
> **说明：** `.gltf` 须与其 `.bin`、贴图等资源放在同一文件夹。必要时，应用会将分离式 glTF 打包为缓存 GLB 以保证预览稳定。
...
通过 **设置**（`⌘,`）调整语言、外观、快捷键与查看器场景选项。快捷键保存在本地，可一键恢复默认。
...
| [window-vibrancy](https://crates.io/crates/window-vibrancy) | macOS 窗口毛玻璃效果 | MIT | [tauri-apps/window-vibrancy](https://github.com/tauri-apps/window-vibrancy) |
